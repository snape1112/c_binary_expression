...
TreeNode * FormulaParser::getTreeRoot() {
  ...
  while (tknzr->advanceToken()) {
    ...
    if (token.type == "constant") { // if constant, add to operands array
      operands.push_back(new ConstantNode(token.content));
    } else if (token.type == "variable") { // if variable, add to operands array
...
std::map<std::string, bool> AssignmentParser::parseAssignment() {
  ...
  // your code starts here
  std::string variable;
  Token lastToken; // to check grammar
  while (tknzr->advanceToken()) {
    Token token = tknzr->getToken();
    if (token.type == "variable") {
      variable = token.content;
    } else if (token.type == "punctuation") {
      if (token.content == ":" && lastToken.type != "variable")
        throw std::runtime_error("invalid input");
      if (token.content == "," && lastToken.type != "constant")
        throw std::runtime_error("invalid input");
    } else if (token.type == "constant") {
      if (lastToken.content != ":")
        throw std::runtime_error("invalid input");
      bool bit = token.content == "1";
      if (results.find(variable) != results.end() && results[variable] != bit)
        throw std::runtime_error("contradicting assignment");
      results[variable] = bit;
      variable.clear();
    } else
      throw std::runtime_error("invalid input");
    lastToken = token;
